...
describe('TodoList', () => {
  ...
  xtest('removeAt removes todo at given index', () => {
  ...
  test('filter returns a new filtered Todo List', () => {
    list.markAllDone();
    let newList = list.filter(todo => todo.isDone() === true);

    expect(newList.isDone()).toBe(true);
  });
